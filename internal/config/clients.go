...
func ListClientCredentials() ([]ClientCredentialsInfo, error) {
	...
	for _, e := range entries {
		...
		name := e.Name()
		switch {
		case name == "registration.json":
			out = append(out, ClientCredentialsInfo{
				Client:  DefaultClientName,
				Path:    filepath.Join(dir, name),
				Default: true,
			})
		case strings.HasPrefix(name, "registration-") && strings.HasSuffix(name, ".json"):
			raw := strings.TrimSuffix(strings.TrimPrefix(name, "registration-"), ".json")

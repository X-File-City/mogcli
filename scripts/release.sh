#!/usr/bin/env bash
# Adapted from opensrc/repos/github.com/steipete/gogcli/scripts/release.sh.
set -euo pipefail
...
if [[ -n "$(git status --porcelain)" ]]; then
  echo "working tree changed by CI checks; commit formatting fixes before releasing" >&2
  exit 2
fi
